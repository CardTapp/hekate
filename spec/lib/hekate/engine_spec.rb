...
RSpec.describe Hekate::Engine do
  ...
  describe "load_environment" do
    describe "online" do
      ...
      it "requests root and environment specific keys" do
        expect_any_instance_of(Hekate::Aws).to receive(:list_parameters).
          with(%w[root development]).
          and_return([double(name: "myapp.root.THREE"), double(name: "myapp.development.ONE"), double(name: "myapp.development.TWO")])
        expect_any_instance_of(Hekate::Aws).to receive(:get_parameters).
          with(["myapp.root.THREE", "myapp.development.ONE", "myapp.development.TWO"]).
          and_return([
      ...
      it "overwrites root keys" do
        expect_any_instance_of(Hekate::Aws).to receive(:list_parameters).
          with(%w[root development]).
          and_return([double(name: "myapp.root.ONE"), double(name: "myapp.development.ONE"), double(name: "myapp.development.TWO")])
        expect_any_instance_of(Hekate::Aws).to receive(:get_parameters).
                                                   with(["myapp.root.ONE", "myapp.development.ONE", "myapp.development.TWO"]).
                                                   and_return([
                                                                  double(name: "myapp.development.ONE", value: "valueone"),
  ...
  describe "put" do
    it "roots items namespaces items correctly" do
      ...
      expect_any_instance_of(Hekate::Aws).to receive(:put_parameter).with("myapp.root.newkey", "123", "root")
